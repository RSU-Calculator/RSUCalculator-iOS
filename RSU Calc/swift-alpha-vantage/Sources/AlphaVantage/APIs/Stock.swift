...
public class Stock: AlphaVantage {
     ...
     */
    public func fuzzyQuery(
        keyword: String,
        completion: @escaping (
            _ result: StockQuerySeries?,
            _ err: Error?
        ) -> Void
    ) {
        
let task = URLSession.shared.stockQuerySeriesTask(with: URL(string: apiUrl(function: .searchEndpoint(keyword: keyword)))!) { stockQuerySeries, response, error in
         if let stockQuerySeries = stockQuerySeries {
            completion(stockQuerySeries, .none)
         }
       }
       task.resume()
    } // - end fuzzyQuery
    ...
    */
    ...
    private func apiUrl(function: ApiConst.Stock.Function) -> String
    {
        return ApiConst.Stock.api(
...
extension URLSession {
    ...
    func stockQuerySeriesTask(with url: URL, completionHandler: @escaping (StockQuerySeries?, URLResponse?, Error?) -> Void) -> URLSessionDataTask {
