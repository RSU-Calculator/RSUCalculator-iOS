...
public struct BarChartView : View {
    ...
    public init(data:ChartData, title: String, legend: String? = nil, style: ChartStyle = Styles.barChartStyleOrangeLight, form: CGSize? = ChartForm.medium, dropShadow: Bool? = true, cornerImage:Image? = Image(systemName: "waveform.path.ecg"), valueSpecifier: String? = "%.1f", currencyFormatter: NumberFormatter, page_headline: String, long_description: String){
        self.data = data
        self.title = title
        self.legend = legend
        self.style = style
...
//                        .foregroundColor(self.colorScheme == .dark ? self.darkModeStyle.legendTextColor : self.style.legendTextColor)
                ...
                BarChartRow(data: data.points.map{$0.1},
                            ...
                            gradient: self.colorScheme == .dark ? self.darkModeStyle.gradientColor : self.style.gradientColor, maxValue: .constant(nil),
...
struct ChartView_Previews : PreviewProvider {
    static var previews: some View {
        BarChartView(data: TestData.values ,
                     title: "Model 3 sales",
                     legend: "Quarterly",
                     valueSpecifier: "%.0f", currencyFormatter: NumberFormatter(), page_headline: "", long_description : "")
    }
}
#endif
